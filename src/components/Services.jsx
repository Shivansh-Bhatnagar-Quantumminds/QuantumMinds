...
const Services = () => {
  ...
  return (
    <div className='min-h-screen w-full px-4 sm:px-6 lg:px-10 py-10'>
      <h1 className='text-xl sm:text-2xl text-[#323d58] text-center font-semibold pt-4 sm:pt-10'>Our Services</h1>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-5 p-2 sm:p-6 lg:px-10 mt-4 sm:mt-6'>
        {data.map((service, index) => (
            <motion.article whileHover={{y:-6}} key={index} className='flex flex-col  p-4 sm:p-6 bg-[#f3fce0] border border-primary/30 rounded shadow-sm'>
